// Standard React + TypeScript version.
// Install dependency: npm install framer-motion
// This component uses the browser's built-in WebGL API.
...
/**
 * MORPHING RINGS — a tilted concentric-ring particle disc for Framer.
 *
 ...
 *   wave.waveAmplitude    -> wave.amplitude  (prefix dropped, rule 8)
 *
 ...
 * shipped 7); it is now rule 8b's signed dial where 50 is that shipped rate.
 *
 * SECOND PASS, clean break, no fallbacks:
 *   baseColor + accentColor + accentMix -> colors[]   (Array of Color, max 5;
 ...
 *
 * Cut and frozen at its current default (rule 8b — the panel row goes, the
...
function __OriginkitBase_MorphingRings(props: Props) {
    const {
        ...
        scale = 38,
        amplitude = 100,
        // a group the designer never opened arrives undefined (rule 8)
    ...
    useEffect(() => {
        ...
        gl.enable(gl.BLEND)
        // Additive on premultiplied: dense inner rings bloom to bright teal-white.
...
/* Framer property controls removed for a standard React app. */

export default function MorphingRings(props: Props) {
    return <__OriginkitBase_MorphingRings {...props} />
}
